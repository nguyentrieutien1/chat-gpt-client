...
import { useMyContext } from "../provider/MyProvider";
...
const ItemHistory = () => {
  ...
  return (
    <div className="relative flex justify-between gap-1 px-3 py-2 bg-[#394053] rounded-full hover:cursor-pointer">
      ...
      {isEdit ? (
        <div className="absolute top-1/2 -translate-y-1/2 right-1 flex items-center gap-2 px-1 bg-[#394053]">
          <HiOutlineCheck
            size={20}
            className="hover:bg-[#595F6F] rounded text-white p-1 transition-all "
            onClick={() => {
              setIsEdit(!isEdit);
