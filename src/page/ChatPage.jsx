...
const ChatPage = () => {
    ...
    useEffect(() => {
        ...
        const intervalId = setInterval(clearLocalStorageArray, 50 * 60 * 1000); // 30 phút = 30 * 60 * 1000 milliseconds
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const result = await axios.post(
                ...
                { data: link.concat(value).concat("(Nếu câu trả lời của bạn là liệt kê, vui lòng đánh dấu là số chứ không phải ** hay * hay bất kì dấu * nào )"), history: conversions }
            );
    ...
    return (
        <div id="form_chat" className="relative h-full sm:w-[90%] mx-auto">
            <div
                ref={messageContainer}
                className="h-chat-message w-full pb-6"
                        ...
                        return (
                            <React.Fragment key={index}>
                                {conversion?.role === "user" ? (
                                    ...
                                    conversion && (
                                        <ChatMessage message={conversion.answer.replace(/\*\*/g, "")} />
                                    )
                                )}
                            </React.Fragment>
                        );
